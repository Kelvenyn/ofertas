...
test("Higienização usa os 37 WebPs normalizados, incluindo favicon e selo", async () => {
  ...
  assert.equal(files.filter((file) => /^page-\d{2}\.webp$/.test(file)).length, 15)
  assert.equal(files.filter((file) => /^testimonial-\d{2}\.webp$/.test(file)).length, 6)
  assert.equal(files.filter((file) => /^bonus-\d{2}-(front|back)\.webp$/.test(file)).length, 12)
  assert.deepEqual(files.filter((file) => /^plan-/.test(file)), ["plan-basic.webp", "plan-complete.webp"])
  assert.ok(files.includes("favicon.webp"))
  assert.ok(files.includes("garantia-30-dias.webp"))
  ...
  for (const line of stdout.trim().split(/\r?\n/)) {
    ...
    if (name.startsWith("plan-")) assert.equal(`${width}x${height}`, "1080x1080")
    else if (name.startsWith("page-") || name.startsWith("bonus-")) assert.ok(Number(width) <= 1200, `${name}: largura acima de 1200px`)
  }
})
