...
**Goal:** Create the main page for `/psicopedagogia` with the same section ordering as `lembrancinhas` but including the `PaletteSwitcher` component at the bottom.

**Context:** This mirrors `src/app/lembrancinhas/page.tsx` but adds `<PaletteSwitcher />` after `<Footer />`.

**Steps:**
1. Create `page.tsx` in `src/app/psicopedagogia/`
2. Run `npx tsc --noEmit src/app/psicopedagogia/page.tsx` to verify
